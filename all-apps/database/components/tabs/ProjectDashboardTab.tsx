    ...
    <div className="animate-fade-in space-y-8">
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
          <StatCard icon={<ChartBarIcon />} title="Schedule Activities" value={project.generatedSchedule?.length.toString() ?? '0'} />
          <StatCard icon={<SparklesIcon />} title="AI Agent Proposals" value={project.agentOutputs?.length.toString() ?? '0'} />
                   ...
                   <div className="md:col-span-2">
                      <h4 className="font-semibold text-base-content mb-2">Selected Features</h4>
                        {Object.keys(project.projectInput.selections).length > 0 ? (
                            ...
                            <p className="text-sm text-base-content-secondary">No specific features selected.</p>
                        )}
                   </div>
              </div>
          </div>
