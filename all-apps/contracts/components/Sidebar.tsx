                ...
                <div className="bg-slate-900/70 p-2 rounded-md max-h-48 overflow-y-auto border border-slate-700">
                    {projects.length === 0 ? (
                        ...
                        <ul className="space-y-1">
                            {projects.map(p => (
                                <li key={p.id} className={`group flex items-center justify-between p-2 rounded-md cursor-pointer transition-colors ${selectedProjectId === p.id ? 'bg-blue-600 text-white' : 'hover:bg-slate-700'}`}>
                                    <span onClick={() => onProjectSelection(p.id)} className="truncate flex-grow" title={p.name}>
                                        {p.name}
                                    </span>
