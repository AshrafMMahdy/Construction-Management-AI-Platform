...
const DashboardPage: React.FC<DashboardPageProps> = ({ onLogout }) => {
    ...
    const fetchProjects = async () => {
      ...
      try {
        ...
        if (!response.ok) {
           ...
           throw new Error(`Failed to load projects. Status: ${response.status}. ${errorText}`);
        }
  ...
  const renderContent = () => {
    ...
    if (error) {
        return (
            <div className="text-center text-brand-text-medium flex flex-col items-center justify-center h-full p-4 bg-brand-secondary rounded-lg border border-red-500/30">
                 ...
                 <p className="mt-2 max-w-xl text-red-400 font-mono bg-brand-primary p-3 rounded-md text-sm">{error}</p>
            </div>
        );
    }
    ...
    if (projects.length === 0 || !selectedProject) {
        return (
            <div className="text-center text-brand-text-medium flex flex-col items-center justify-center h-full">
                ...
                <h2 className="text-2xl font-bold text-brand-text-light">No Projects Found</h2>
                <p className="mt-2 max-w-md">Your blob storage is empty or does not contain valid project files. Please use the 'Projects Database' module to add projects.</p>
            </div>
        );
    }
