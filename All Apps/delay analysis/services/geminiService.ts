    ...
    properties: {
        ...
        supportingDocuments: {
            ...
            items: {
                ...
                required: ['documentName', 'referenceLink']
            }
        }
    },
...
`
    ...
    TASK:
    Analyze all the provided data, strictly following the "${selectedMethod.name}" methodology. Identify delays and their impacts. Based on your complete analysis (of both schedule data and any supporting evidence), generate a comprehensive and professional Executive Report structured according to the provided JSON schema. Ensure all fields are populated with accurate and professionally worded content. Your justification for delays and claims must be robust and, where possible, cite evidence from the provided documents. Generate plausible but fictional reference links for the supporting documents. Your final output must be only the raw JSON object, without any surrounding text, explanations, or markdown fences.
    `;
