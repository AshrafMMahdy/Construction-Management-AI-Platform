...
function App() {
  ...
  const handleAnalyze = useCallback(async () => {
    ...
    try {
      ...
      if (!contractClauses || contractClauses.length === 0) throw new Error("Could not extract any valid clauses from the contract file.");
  ...
  const handleSearch = useCallback(async () => {
    ...
    try {
        ...
        const results = await searchContract(contractClauses, searchQuery);
