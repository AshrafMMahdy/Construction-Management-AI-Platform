...
const evaluateAgentProposal = (proposal: AgentActivitySuggestion[]): EvaluationScores => {
    if (!proposal || proposal.length === 0) return { "Overall Status": "No data", "Total Activities": 0 };
    
    const totalDuration = proposal.reduce((sum, item) => sum + (Number(item.duration) || 0), 0);
