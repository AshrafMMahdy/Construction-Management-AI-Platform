...
import { Activity, AgentOutput, ProjectInput, ProjectFeatures, ProgressStep, ProgressUpdate } from './types';
...
const App: React.FC = () => {
  ...
  const handleFileLoad = useCallback((content: string, name: string) => {
    ...
    setProgress([]); // Clear progress from any previous runs
  ...
  const handleProgressUpdate = (updates: ProgressUpdate[]) => {
    setProgress(currentProgress => {
        ...
        updates.forEach(update => {
            // Find the step by its original name, even if it has been updated with an attempt counter.
            const stepIndex = newProgress.findIndex(p => p.name.startsWith(update.name));
            if (stepIndex !== -1) {
                newProgress[stepIndex] = { 
                    ...newProgress[stepIndex], 
                    // Use the new name if provided, otherwise keep the current name.
            ...
            {generatedSchedule ? (
              <div className="flex flex-col gap-8">
               ...
               <ProgressDisplay 
                 ...
                 title={isLoading ? "Generating Your Schedule..." : "Generation Failed"}
               />
            ) : (
