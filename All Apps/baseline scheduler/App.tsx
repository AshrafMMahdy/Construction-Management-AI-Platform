...
const LOCAL_STORAGE_KEY = 'ai_construction_projects';
...
const App: React.FC = () => {
      ...
      if (projectToLoad && !isLoading) {
          ...
          try {
              ...
              if (projectToLoad.generatedSchedule && projectToLoad.generatedSchedule.length > 0) {
                  const calculatedGanttData = calculateScheduleForwardPass(projectToLoad.generatedSchedule, projectToLoad.startDate);
                  setGanttData(calculatedGanttData);
                  calculateAndSetDuration(calculatedGanttData);
                  setView('gantt');
              }
  ...
  const handleConfirmSaveAndReset = () => {
    const savedSuccessfully = handleSaveProject();
